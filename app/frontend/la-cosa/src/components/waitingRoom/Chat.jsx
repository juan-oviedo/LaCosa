...
function Chat({gameID, userID, wsMsg}) {
  ...
  //     try {
  //       const response = await axios.get(`http://127.0.0.1:8000/chat/get_messages/${gameID}`);
  ...
  const fetchNewMessage = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/chat/get_messages_from/${gameID}/${lastmsg}`);
      setMessages(response.data);
      setLastmsg(1);
      console.log(lastmsg);
    } catch (error) { 
