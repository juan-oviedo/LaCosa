...
function PageGame() {
  ...
  const recoverLastWsMessage = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8000/game/lastMessage?id_game=${gameID}&id_player=${userID}`
      );
  ...
  const handleQuarantine = async (player_id) => {
    try{
      const response = await axios.get(
        `http://127.0.0.1:8000/card/show_card/${player_id}`
      );
      console.log(response.data);
      if (response.status === 200) {
...
useEffect(() => {
  ...
  const sendRevelacion = (show_cards,show_infection) =>{
    const url="http://127.0.0.1:8000/card/revelaciones";
    const data = {
      id_player: userID,
  ...
  const sendRevelacionInfection = (playerId) =>{
    const url = `http://localhost:8000/card/show_infection?id_player=${playerId}`;
    axios.post(url)
    .then((response) => {
      if(response.status === 200)
      console.log(response.data);
